                                                            ...
                                                            if(subcateg != null) {
                                                                if (!req.files || Object.keys(req.files).length === 0) {
                                                                    if(!Content) {
                                                                        ...
                                                                        ActualityModel.findByIdAndUpdate(ObjectId, dataActuality, function(err, created) {
                                                                            if(err) {
                                                                                console.log(err);
                                                                                return res.json({response: 'Un problème interne est survenue', status: 'error'});
                                                                            } else {
                                                                                return res.json({response: 'Votre actualité a bien été modifié', status: 'success'});
                                                                            }
                                                                        })
                                                                    }
                                                                } else {
