...
class HomeNewsCoopComponent extends React.Component {
    ...
    requestNews = () => {
        ...
        fetch("https://api.mwactu.fr/v1/news/?category=2&limit=ok")
            .then(res => res.json())
            .then(
                (result) => {
                    this.setState({
    ...
    render() {
        ...
        return (
            <View style={{marginBottom: 15}}>
                ...
                <Text style={{color: themeMode == 'dark' ? '#fff' : '#000', fontWeight: '600', fontSize: 22}}>Actualités Coop</Text>
                    <TouchableOpacity onPress={() => this.props.navigation.navigate('NewsAll', {idCategory: 2})} style={{paddingLeft: 10, paddingRight: 10, backgroundColor: 'rgba(88,88,88,0.48)', marginLeft: 10, borderRadius: 25 / 2, alignItems: 'center', justifyContent: 'center'}}>
